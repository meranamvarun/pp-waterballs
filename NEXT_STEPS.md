...
- **Buy Me a Coffee link** added to every footer, pointing at `buymeacoffee.com/ppwaterballs`.
...
3. **Buy Me a Coffee.** Create the account and confirm the username matches
   `ppwaterballs` — the link currently in every footer is a placeholder guess, not a
   verified page.
4. **Print-on-demand merch store.** Design the 2-3 t-shirt/mug/sticker graphics (text
   ...
   product links.
5. **Domain renewal.** Confirm it's cleared in Hostinger — outside this repo entirely.
6. **Newsletter/social account** riffing on "craziest IPO of the week" — a separate
